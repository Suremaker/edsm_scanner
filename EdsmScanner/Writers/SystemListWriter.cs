...
namespace EdsmScanner.Writers
{
    internal class SystemListWriter
    {
        ...
        {
            await writer.WriteLineAsync($"{sys.Ref.Name} [{distance:F2}ly] ({sys.Ref.BodyCount?.ToString() ?? "?"} bodies / {sys.Bodies?.Length.ToString() ?? "?"} discovered / {sys.StarCount} stars discovered) => {sys.Url}");
        }
